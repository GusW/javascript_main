...
user = {
    ...
    profile() {
        return Meteor.user().profile.access
    },
    username() {
    ...
    lastAccess() {
        return dateFormat(Meteor.user().profile.lastSuccess, "UTC:mm/dd/yyyy HH:MM:ss");
    }
};
...
Template.userInfo.events({
    'click #changePass': function(event) {
        const pass01 = $("#pass01").val();
        const pass02 = $("#pass02").val();
        if (pass01 == "" && pass02 == "") {
            alertUser.type = "alert-danger";
            alertUser.message = "Please type a new Password.";
            const alertClass = $(".alertUser");
            showHideUserAlert(alertClass);
        } else if (pass01.length < 6 && pass02.length < 6) {
            alertUser.type = "alert-warning";
            alertUser.message = "Password must have at least 6 characters.";
            showHideUserAlert(alertClass);
        } else if (pass01 != pass02) {
            alertUser.type = "alert-warning";
            alertUser.message = "The passwords don't match. Please verify and try again.";
            const alertClass = $(".alertUser");
            showHideUserAlert(alertClass);
        } else {
            const id = user.id;
            Meteor.call('users.setPass', id, pass01);
            alertUser.type = "alert-success";
            alertUser.message = "The password was successfully changed.";
            const alertClass = $(".alertUser");
            showHideUserAlert(alertClass);
            $("#pass01").val("");
            $("#pass02").val("");
